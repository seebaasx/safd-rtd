...
import { 
  ...
  Send, Clock, Calendar, ThumbsUp, ThumbsDown, Edit2, Check
} from 'lucide-react';
...
export default function App() {
  ...
  const [isEditingRango, setIsEditingRango] = useState(false);
  const [tempRango, setTempRango] = useState('');
  ...
  useEffect(() => {
    if (selectedStudent) {
      ...
      setTempRango(selectedStudent.rango || 'Rango 0');
    }
  }, [selectedStudent]);

  const updateStudentData = async (column, value) => {
    const today = "5/5/2026"; // Fecha fija según tu Canvas
    ...
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col md:flex-row font-sans">
      <aside className="w-full md:w-24 bg-[#09090b] border-r border-zinc-800/50 flex flex-col items-center py-10 h-screen sticky top-0">
        ...
        <nav className="flex md:flex-col gap-8">
          <button onClick={() => { setActiveTab('alumnos'); setSelectedStudent(null); }} className={`p-4 rounded-2xl ${activeTab === 'alumnos' ? 'bg-red-600 text-white' : 'text-zinc-600'}`}><Users /></button>
          <button onClick={() => setActiveTab('progreso')} className={`p-4 rounded-2xl ${activeTab === 'progreso' ? 'bg-red-600 text-white' : 'text-zinc-600'}`}><BarChart3 /></button>
          <button onClick={() => setActiveTab('recursos')} className={`p-4 rounded-2xl ${activeTab === 'recursos' ? 'bg-red-600 text-white' : 'text-zinc-600'}`}><BookOpen /></button>
        </nav>
        ...
        {selectedStudent ? (
          <div className="space-y-12 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-[2.5rem] p-10">
                <div className="flex justify-between items-center mb-6">
                   <span className="text-zinc-600 text-[9px] font-black uppercase tracking-widest italic">Aprobación Técnica</span>
            ...
            <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-[2.5rem] p-10 border-t-4 border-t-blue-600">
               ...
               {selectedStudent.voto_instructor && (
                 <div className="bg-green-950/20 border border-green-900/30 p-6 rounded-2xl mb-8 flex justify-between items-center">
                    ...
                    <div className="px-4 py-1 rounded-full text-[8px] font-black bg-green-900/40 text-green-500 border border-green-600/20 uppercase">APTO</div>
                 </div>
               )}
               <div className="grid grid-cols-2 gap-4">
                  <button onClick={() => updateStudentData('voto_instructor', 'apto')} className="flex flex-col items-center gap-2 p-8 bg-zinc-950/50 border border-zinc-800 rounded-3xl hover:border-green-600 transition-all group">
                    <ThumbsUp className="w-6 h-6 text-zinc-700 group-hover:text-green-500" /><span className="text-[9px] font-black text-zinc-700 group-hover:text-white uppercase tracking-widest">APTO</span>
                  </button>
                  <button onClick={() => updateStudentData('voto_instructor', 'no_apto')} className="flex flex-col items-center gap-2 p-8 bg-zinc-950/50 border border-zinc-800 rounded-3xl hover:border-red-600 transition-all group">
                    <ThumbsDown className="w-6 h-6 text-zinc-700 group-hover:text-red-500" /><span className="text-[9px] font-black text-zinc-700 group-hover:text-white uppercase tracking-widest">NO APTO</span>
                  </button>
               </div>
            </div>
            ...
            <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-[2.5rem] p-10 border-t-4 border-t-red-600">
               ...
               <div className="space-y-4 mb-10">
                  {observations.map(obs => (
                    <div key={obs.id} className="bg-zinc-950/50 border border-zinc-800 rounded-2xl p-6 relative">
                       <div className="flex justify-between items-center mb-4 text-[10px] font-black">
                          <div className="flex items-center gap-2 italic"><div className="w-1.5 h-1.5 bg-red-600 rounded-full animate-pulse"></div> {obs.instructor_name}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">
            {students.map(s => (
              <div key={s.id} onClick={() => setSelectedStudent(s)} className="group bg-zinc-900/20 border border-zinc-800/40 p-10 rounded-[3rem] hover:border-red-600 transition-all cursor-pointer">
                ...
                <p className="text-[10px] font-black text-zinc-700 uppercase tracking-[0.3em] group-hover:text-red-500 transition-all">{s.rango || 'Rango 0'}</p>
              </div>
            ))}
          </div>
        )}

...
const supabaseUrl = 'bwisxczbkjlxyunpqqld.supabase.co';
const supabaseKey = 'sb_publishable_MEosBztTd-5Ot5Rb-jhaHg_BEeiWZ19';
const supabase = createClient(supabaseUrl, supabaseKey);

// --- 🛡️ CONFIGURACIÓN DE PODERES ADMIN ---
// Añade aquí los "usuarios" (emails) de las 2 personas que quieres que sean ADMINS
...
export default function App() {
  ...
  if (loading) return <div className="min-h-screen bg-zinc-950 flex items-center justify-center text-red-600 font-black">SINCRO...</div>;
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col font-sans overflow-hidden">
        <nav className="fixed top-0 w-full z-50 px-12 py-8 flex justify-between items-center bg-gradient-to-b from-black/80 to-transparent">
          <div className="flex items-center gap-4"><Flame className="w-10 h-10 text-red-600" /><span className="text-2xl font-black text-white italic">SAFD RTD</span></div>
          <button onClick={() => setHasEntered(true)} className="px-8 py-3 bg-red-600 text-white rounded-full font-black uppercase text-xs">Acceder</button>
        </nav>
          ...
          {slides.map((slide, i) => (
            <div key={i} className={`absolute inset-0 transition-opacity duration-1000 ${i === currentSlide ? 'opacity-100' : 'opacity-0'}`}>
              ...
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center">
                <h2 className="text-8xl font-black text-white italic mb-4">{slide.title}</h2>
                <p className="text-zinc-400 italic mb-10">{slide.subtitle}</p>
                <button onClick={() => setHasEntered(true)} className="bg-white text-black px-12 py-5 rounded-full font-black uppercase hover:bg-red-600 hover:text-white transition-all">Abrir Sistema</button>
              </div>
            </div>
          ))}
        </div>
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-6">
        <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-[2.5rem] p-10 shadow-2xl">
          <div className="flex flex-col items-center mb-10"><Flame className="w-12 h-12 text-red-600 mb-4" /><h1 className="text-3xl font-black text-white italic">PORTAL SAFD</h1></div>
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="text" placeholder="Usuario (ej: pablo@safd.com)" className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl py-4 px-6 text-white outline-none focus:border-red-600" value={emailInput} onChange={e => setEmailInput(e.target.value)} required />
            <input type="password" placeholder="Contraseña" className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl py-4 px-6 text-white outline-none focus:border-red-600" value={passInput} onChange={e => setPassInput(e.target.value)} required />
            <button type="submit" className="w-full bg-red-600 hover:bg-red-500 text-white py-4 rounded-xl font-black uppercase transition-all active:scale-95">Entrar</button>
            <button type="button" onClick={() => setHasEntered(false)} className="w-full text-zinc-600 text-[10px] uppercase font-black mt-4">Volver al Inicio</button>
          </form>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-sans flex flex-col md:flex-row">
      ...
      <main className="flex-1 overflow-y-auto px-6 py-12 md:px-16 md:py-16">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-16">
          <div>
            <h1 className="text-6xl font-black text-white uppercase italic tracking-tighter leading-none">{activeTab === 'alumnos' ? 'Expedientes' : activeTab === 'progreso' ? 'Estadísticas' : 'Biblioteca'}</h1>
            <p className="text-[10px] text-zinc-500 font-black uppercase mt-5 flex items-center gap-3">
              <Activity className="w-4 h-4 text-red-600 animate-pulse" /> Operativo: <span className="text-white underline decoration-red-900 decoration-4">Inst. {instructorName}</span>
              {isAdmin && <span className="bg-blue-600 text-white text-[8px] px-2 py-0.5 rounded-full font-black">ADMIN</span>}
            </p>
          </div>
          {activeTab === 'alumnos' && !selectedStudentId && isAdmin && (
            <button onClick={() => setIsAddingStudent(!isAddingStudent)} className="bg-white text-black hover:bg-red-600 hover:text-white px-10 py-5 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all">
              {isAddingStudent ? 'Volver' : 'Nuevo Aspirante'}
            </button>
          )}
        </div>

        {/* ... (Resto del Dashboard idéntico al anterior) ... */}
        <div className="p-4 text-zinc-700 text-[10px] italic">Sistema de Gestión de Personal SAFD v2.0 - Supabase Auth</div>
      </main>
    </div>
  );
}
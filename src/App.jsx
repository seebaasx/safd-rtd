...
export default function App() {
  ...
  const handleCreateResource = async (e) => {
    ...
    // Guardamos solo título y url para evitar errores de columnas inexistentes
    const { error } = await supabase.from('resources').insert([{ title: newRes.title, url: newRes.url }]);
                  ...
                  <div className="space-y-6 mb-12 max-h-[400px] overflow-y-auto pr-4 custom-scrollbar">
                     {observations.map(obs => (
                       <div key={obs.id} className="bg-black/40 border border-white/5 rounded-3xl p-8 shadow-inner group relative">
                          <div className="flex justify-between items-center mb-4 border-b border-white/5 pb-4"><div className="flex items-center gap-3 italic"><div className="w-1.5 h-1.5 bg-red-600 rounded-full animate-pulse shadow-md shadow-red-600/50"></div><span className="text-[10px] font-black text-white">{obs.instructor_name}</span></div><span className="text-[8px] text-zinc-700 font-black uppercase">F. REGISTRO</span></div>

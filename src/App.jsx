        ...
        {slides.map((slide, i) => (
          <div key={i} className={`absolute inset-0 transition-opacity duration-1000 ${i === currentSlide ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center px-6">
              <h2 className="text-7xl md:text-[9rem] font-black italic mb-2 tracking-tighter leading-none animate-in fade-in zoom-in duration-700">{slide.title}</h2>
    ...
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col md:flex-row font-sans">
      <aside className="w-full md:w-24 bg-[#09090b] border-r border-zinc-800/50 flex flex-col items-center py-10 h-screen sticky top-0 z-50">
        ...
        <nav className="flex md:flex-col gap-8">
          <button disabled={!!selectedStudent} onClick={() => { setActiveTab('alumnos'); setSelectedStudent(null); }} className={`p-4 rounded-2xl transition-all ${selectedStudent ? 'opacity-10' : 'hover:scale-110'} ${activeTab === 'alumnos' ? 'bg-red-600 text-white shadow-xl' : 'text-zinc-600'}`}><Users /></button>
          <button disabled={!!selectedStudent} onClick={() => setActiveTab('progreso')} className={`p-4 rounded-2xl transition-all ${selectedStudent ? 'opacity-10' : 'hover:scale-110'} ${activeTab === 'progreso' ? 'bg-red-600 text-white shadow-xl' : 'text-zinc-600'}`}><BarChart3 /></button>
          <button disabled={!!selectedStudent} onClick={() => setActiveTab('recursos')} className={`p-4 rounded-2xl transition-all ${selectedStudent ? 'opacity-10' : 'hover:scale-110'} ${activeTab === 'recursos' ? 'bg-red-600 text-white shadow-xl' : 'text-zinc-600'}`}><BookOpen /></button>
        </nav>
      ...
      <main className="flex-1 p-6 md:p-16 overflow-y-auto">
        ...
        {selectedStudent ? (
          /* --- VISTA EXPEDIENTE ALUMNO (SIN CAMBIOS) --- */
          <div className="space-y-12 pb-20 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
               <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-[3rem] p-10">
                  ...
                  {isEditingHorario ? <input className="bg-zinc-950 border border-zinc-800 text-white p-2 rounded w-full font-black uppercase" value={tempHorario} onChange={e => setTempHorario(e.target.value)} /> : <div className="text-xl font-black italic border-b border-zinc-800 pb-4">{selectedStudent.horario || 'MAÑANA / TARDE'}</div>}
               </div>
               <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-[3rem] p-10">
                  <div className="text-zinc-600 text-[9px] font-black uppercase tracking-widest mb-6">Rango Académico</div>
                  <select className="bg-zinc-950 border border-zinc-800 text-white p-3 rounded-xl w-full font-black uppercase outline-none focus:border-red-600" value={selectedStudent.rango || "Rango 0"} onChange={(e) => updateStudentData('rango', e.target.value)}>{RANGOS_ACADEMIA.map(r => <option key={r} value={r}>{r.toUpperCase()}</option>)}</select>
               </div>
               <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-[3rem] p-10">
                  <div className="flex justify-between items-center mb-6"><span className="text-zinc-600 text-[9px] font-black uppercase tracking-widest">Aprobación Técnica</span><span className="text-red-600 font-black italic text-xl">43%</span></div>
            ...
            <div className="space-y-4">
              ...
              {[ { key: 'actitud', label: 'ACTITUD' }, { key: 'mando', label: 'MANDO' }, { key: 'interna', label: 'BUEN USO DE INTERNA' }, { key: 'radio', label: 'RADIO' }, { key: 'primeros_aux', label: 'AUXILIOS' }, { key: 'excarcelacion_hab', label: 'EXCARCELACIÓN' }, { key: 'incendios_hab', label: 'INCENDIOS' } ].map((skill) => (
                <div key={skill.key} className="bg-zinc-900/30 border border-zinc-800/60 p-8 rounded-[2rem] flex flex-col md:flex-row justify-between md:items-center gap-6">
                  <div><div className="font-black italic text-xl uppercase mb-2">{skill.label}</div><div className="text-[8px] font-black text-zinc-600 uppercase tracking-widest">{selectedStudent[skill.key] && selectedStudent[skill.key] !== 'no' ? `VALIDADO: ${selectedStudent[`${skill.key}_validador`]}` : 'Pte. Validación'}</div></div>
                  <div className="flex gap-2">
                    {['no', 'cursando', 'aprendido'].map(status => (
                      <button key={status} onClick={() => updateStudentData(skill.key, status)} className={`px-6 py-2 rounded-xl text-[9px] font-black transition-all ${selectedStudent[skill.key] === status ? (status === 'aprendido' ? 'bg-green-600 text-white' : status === 'cursando' ? 'bg-yellow-600 text-white' : 'bg-zinc-700 text-white') : 'bg-zinc-800/30 text-zinc-600'}`}>{status.toUpperCase()}</button>
                    ))}
                  </div>
                </div>
              ))}
            ...
            <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-[3rem] p-10 border-t-4 border-t-red-600">
               ...
               <div className="space-y-6 mb-12 max-h-[400px] overflow-y-auto pr-4 custom-scrollbar">
                  {observations.map(obs => (
                    <div key={obs.id} className="bg-zinc-950/90 border border-zinc-800 rounded-3xl p-8 shadow-lg">
                       <div className="flex justify-between items-center mb-4 border-b border-zinc-900 pb-4">
                          ...
                          <span className="text-[8px] text-zinc-700 font-black uppercase">5/5/2026</span>
                       </div>
                       <p className="pl-6 border-l-2 border-red-600/40 text-zinc-400 italic text-sm leading-relaxed">{obs.content}</p>
                    </div>
                  ))}
               </div>
               <div className="bg-zinc-950 border border-zinc-800 rounded-[2.5rem] p-4 flex items-center gap-4">
                  <textarea value={newObs} onChange={e => setNewObs(e.target.value)} placeholder="Redactar..." className="bg-transparent flex-1 outline-none p-4 text-zinc-300 resize-none h-24 text-sm italic" />
                  <button onClick={sendObservation} className="bg-red-600 p-5 rounded-full text-white"><Send className="w-6 h-6" /></button>
               </div>
            </div>
          </div>
        ) : (
          ...
          <div className="animate-in fade-in duration-500">
            ...
            {activeTab === 'progreso' && (
              <div className="space-y-12">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-[2.5rem] p-10"><TrendingUp className="text-zinc-600 mb-6" /><div className="text-6xl font-black italic">{students.length}</div><div className="text-[10px] font-black text-zinc-600 uppercase">Aspirantes</div></div>
                  <div className="bg-zinc-900/30 border border-green-900/30 rounded-[2.5rem] p-10"><CheckCircle2 className="text-green-600 mb-6" /><div className="text-6xl font-black italic text-green-500">{students.filter(s => s.voto_instructor === 'apto').length}</div><div className="text-[10px] font-black text-zinc-600 uppercase">Graduados</div></div>
                  <div className="bg-zinc-900/30 border border-yellow-900/30 rounded-[2.5rem] p-10"><AlertCircle className="text-yellow-600 mb-6" /><div className="text-6xl font-black italic text-yellow-500">{students.filter(s => s.voto_instructor === null).length}</div><div className="text-[10px] font-black text-zinc-600 uppercase">En Prueba</div></div>
                </div>
              </div>
            )}
            ...
            {activeTab === 'recursos' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {resources.map(r => (
                  <div key={r.id} className="group bg-zinc-900/20 border border-zinc-800/40 p-10 rounded-[3rem] hover:border-blue-600/50 transition-all relative shadow-xl">
                    ...
                    <a href={r.url} target="_blank" rel="noreferrer" className="inline-flex h-14 items-center px-10 bg-zinc-950 border border-zinc-800 rounded-2xl text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-white hover:bg-blue-600 transition-all">ABRIR DOCUMENTO</a>
                  </div>
                ))}
              </div>
            )}
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/90 backdrop-blur-md">
            <div className="bg-[#09090b] border border-zinc-800 w-full max-w-xl rounded-[3.5rem] p-16 shadow-2xl">
              <div className="flex justify-between items-center mb-12"><h2 className="text-4xl font-black italic uppercase tracking-tighter text-white">Alta Aspirante</h2><button onClick={() => setIsModalOpen(false)} className="text-zinc-700 hover:text-white"><X className="w-8 h-8" /></button></div>
              <form onSubmit={handleCreateStudent} className="space-y-10">
                <input type="text" className="w-full bg-zinc-900 border border-zinc-800 rounded-2xl py-6 px-10 outline-none focus:border-red-600 transition-all font-black uppercase italic text-white text-xl" value={newStudentName} onChange={e => setNewStudentName(e.target.value)} placeholder="NOMBRE COMPLETO" required />
                <button type="submit" className="w-full bg-red-600 py-7 rounded-2xl font-black uppercase text-[11px] shadow-2xl shadow-red-600/20 text-white">REGISTRAR</button>
              </form>
            </div>
          </div>
        )}
        ...
        {isResModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/90 backdrop-blur-md">
            <div className="bg-[#09090b] border border-zinc-800 w-full max-w-xl rounded-[3.5rem] p-16 shadow-2xl">
              <div className="flex justify-between items-center mb-12"><h2 className="text-4xl font-black italic uppercase tracking-tighter text-white">Nuevo Recurso</h2><button onClick={() => setIsResModalOpen(false)} className="text-zinc-700 hover:text-white"><X className="w-8 h-8" /></button></div>
              <form onSubmit={handleCreateResource} className="space-y-6">
                <input type="text" className="w-full bg-zinc-900 border border-zinc-800 rounded-2xl py-6 px-10 text-white font-black italic uppercase outline-none focus:border-blue-600" value={newRes.title} onChange={e => setNewRes({...newRes, title: e.target.value})} placeholder="TÍTULO" required />
                <input type="url" className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl py-6 px-10 text-white font-black italic outline-none focus:border-blue-600" value={newRes.url} onChange={e => setNewRes({...newRes, url: e.target.value})} placeholder="URL" required />

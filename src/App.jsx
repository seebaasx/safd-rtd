...
export default function App() {
  ...
  const slides = [
    ...
    { title: "ACADEMY OPS", subtitle: "SISTEMA DE EVALUACIÓN TÁCTICA", image: "https://r2.fivemanage.com/rlMpa4HCjCLM3vQVrxiNo/imagen_2026-04-13_224256139.png" }
  ];
    ...
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col md:flex-row font-sans">
      <aside className="w-full md:w-24 bg-[#09090b] border-r border-zinc-800/50 flex flex-col items-center py-10 h-screen sticky top-0 z-50">
        ...
        <nav className="flex md:flex-col gap-8">
          <button disabled={!!selectedStudent} onClick={() => { setActiveTab('alumnos'); setSelectedStudent(null); }} className={`p-4 rounded-2xl transition-all ${selectedStudent ? 'opacity-10 cursor-not-allowed' : 'hover:scale-110'} ${activeTab === 'alumnos' ? 'bg-red-600 text-white' : 'text-zinc-600 hover:text-white'}`}><Users className="w-6 h-6" /></button>
          <button disabled={!!selectedStudent} onClick={() => setActiveTab('progreso')} className={`p-4 rounded-2xl transition-all ${selectedStudent ? 'opacity-10 cursor-not-allowed' : 'hover:scale-110'} ${activeTab === 'progreso' ? 'bg-red-600 text-white' : 'text-zinc-600 hover:text-white'}`}><BarChart3 className="w-6 h-6" /></button>
          <button disabled={!!selectedStudent} onClick={() => setActiveTab('recursos')} className={`p-4 rounded-2xl transition-all ${selectedStudent ? 'opacity-10 cursor-not-allowed' : 'hover:scale-110'} ${activeTab === 'recursos' ? 'bg-red-600 text-white' : 'text-zinc-600 hover:text-white'}`}><BookOpen className="w-6 h-6" /></button>
        </nav>
      ...
      <main className="flex-1 p-6 md:p-16 overflow-y-auto">
        ...
        {selectedStudent ? (
          <div className="space-y-12 pb-20 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
               <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-[3rem] p-10">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <button onClick={() => isEditingHorario ? (updateStudentData('horario', tempHorario), setIsEditingHorario(false)) : setIsEditingHorario(true)}><Edit2 className="w-4 h-4" /></button>
                  </div>
                  {isEditingHorario ? <input className="bg-zinc-950 border border-zinc-800 text-white p-2 rounded w-full font-black uppercase" value={tempHorario} onChange={e => setTempHorario(e.target.value)} /> : <div className="text-xl font-black italic border-b border-zinc-800 pb-4 uppercase">{selectedStudent.horario}</div>}
               </div>
               <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-[3rem] p-10">
            ...
            <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-[3rem] p-10 border-t-4 border-t-green-600">
                ...
                <tbody className="text-[10px] font-black uppercase italic text-center">
                  {[ { key: 'asis_radio', label: 'RADIO & DISPATCH' }, { key: 'asis_auxilios', label: 'PRIMEROS AUXILIOS' }, { key: 'asis_incendios', label: 'INCENDIOS' }, { key: 'asis_excarcelacion', label: 'EXCARCELACIÓN' } ].map(mod => (
                    <tr key={mod.key} className="bg-zinc-950/20"><td className="py-5 px-4 text-zinc-400 text-left">{mod.label}</td>
                      {['p', 'a', 'r'].map(type => (
                        <td key={type} className="py-5"><button onClick={() => updateStudentData(mod.key, type)} className={`w-5 h-5 rounded-md mx-auto border transition-all ${selectedStudent[mod.key] === type ? (type === 'p' ? 'bg-green-500 border-green-400' : type === 'a' ? 'bg-red-500 border-red-400' : 'bg-blue-600 border-blue-400') : 'bg-zinc-900 border-zinc-800'}`} /></td>
                      ))}
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
               <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-[3rem] p-10 border-t-4 border-t-blue-600">
                  <div className="text-zinc-300 text-[10px] font-black uppercase tracking-widest mb-10 italic">Votación Instructores</div>
                  {selectedStudent.voto_instructor && <div className="bg-green-950/10 border border-green-900/30 p-6 rounded-2xl mb-8 flex justify-between items-center animate-pulse"><div className="text-[10px] font-black">{instructorInfo.fullTag}</div><div className={`px-4 py-1 rounded-full text-[8px] font-black uppercase ${selectedStudent.voto_instructor === 'apto' ? 'bg-green-900/40 text-green-500' : 'bg-red-900/40 text-red-500'}`}>{selectedStudent.voto_instructor.toUpperCase()}</div></div>}
                  <div className="grid grid-cols-2 gap-4">
                     <button onClick={() => updateStudentData('voto_instructor', 'apto')} className={`p-8 bg-zinc-950/50 border rounded-3xl transition-all flex flex-col items-center gap-2 ${selectedStudent.voto_instructor === 'apto' ? 'border-green-600 bg-green-600/10' : 'border-zinc-800 hover:border-green-600'}`}><ThumbsUp className="w-6 h-6 text-green-500" /><span className="text-[9px] font-black uppercase">APTO</span></button>
                     <button onClick={() => updateStudentData('voto_instructor', 'no_apto')} className={`p-8 bg-zinc-950/50 border rounded-3xl transition-all flex flex-col items-center gap-2 ${selectedStudent.voto_instructor === 'no_apto' ? 'border-red-600 bg-red-600/10' : 'border-zinc-800 hover:border-red-600'}`}><ThumbsDown className="w-6 h-6 text-red-500" /><span className="text-[9px] font-black uppercase">NO APTO</span></button>
                  </div>
               </div>
               ...
               <div className="bg-zinc-900/30 border border-zinc-800/60 rounded-[3rem] p-10 border-t-4 border-t-red-600">
                     ...
                     {observations.map(obs => (
                       <div key={obs.id} className="bg-zinc-950/90 border border-zinc-800 rounded-3xl p-8 shadow-lg group hover:border-red-600/30 transition-all">
                          <div className="flex justify-between items-center mb-4 border-b border-zinc-900 pb-4">
                             <div className="flex items-center gap-3 italic"><div className="w-1.5 h-1.5 bg-red-600 rounded-full animate-pulse"></div><span className="text-[10px] font-black text-white">{obs.instructor_name}</span></div>
                  ...
                  <div className="bg-zinc-950 border border-zinc-800 rounded-[2.5rem] p-4 flex items-center gap-4">
                     ...
                     <button onClick={sendObservation} className="bg-red-600 p-5 rounded-full text-white hover:scale-110 transition-all"><Send className="w-6 h-6" /></button>
                  </div>
               </div>
            </div>
          </div>
          ...
          <div className="animate-in fade-in duration-500">
            ...
            {activeTab === 'recursos' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {resources.map(r => (
                  <div key={r.id} className="group bg-zinc-900/20 border border-zinc-800/40 p-10 rounded-[3rem] hover:border-blue-600/50 transition-all relative shadow-xl overflow-hidden shadow-black/20">
